...
- Each deterministic candidate in the `deterministicCandidates` array carries a `mismatchReasons` list when it did not match. Each entry has a `dimension` (`query`, `header`, `scenario`, or `response`), a `key` identifying which parameter or header failed, the `expected` value from the stub definition, the `actual` value from the request (or `null` when the key was absent), and a `kind` of `missing`, `unequal`, or `notConfigured`. The list is empty for candidates that matched or were selected.
